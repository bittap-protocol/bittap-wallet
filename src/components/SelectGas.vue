<template>
    <div class="select-gas">
        ...
        <button class="checkBtn" @click="showGasDialog">
            <div class="show">
                {{ showCate }}
            </div>
            <div class="arrow">
      ...
      @input="$emit('update:modelValue', Number($event.target.value))" type="number" min="1" placeholder="Custom gas fee" />
            <div class="badge badge-primary sat">Sats</div>
        </div>
    </div>

    <dialog :id="dialogId" class="modal rounded-sm">
        <div class="modal-box dialog">
            ...
            <div class="cb">
                <div role="tablist" class="tabs tabs-bordered">
                    <a role="tab" :class="['tab', tabActive === 'Recommend' ? 'tab-active' : '']" @click="tabActive = 'Recommend'">Recommend</a>
                    <a role="tab" :class="['tab', tabActive === 'Custom' ? 'tab-active' : '']" @click="tabActive = 'Custom'">Custom</a>
                </div>
                <div class="choose" v-if="tabActive === 'Recommend'">
                    <label v-for="(item, index) in categories" :key="index" :class="['item', formDataGas === item.gas ? 'active':'']" @click="setFormDataGas(item)">
                        <div class="iconInfo">
                        ...
                        <div class="ch">
                            <input type="checkbox" :checked="formDataGas === item.gas" class="checkbox checkbox-primary" />
                        </div>
                    </label>
                </div>
                 <div class="custom" v-if="tabActive === 'Custom'">
                    <div class="name">SATS</div>
                    <input class="field" v-model="formDataGas"  type="number" min="1" placeholder="Enter sats" />
                 </div>
            </div>
            <div class="dialog-actions">
                <button class="btn btn-primary" @click="setGasConfirm">Confirm</button>
            </div>
        </div>
    </dialog>
...
export default {
    name: 'SelectGas',
    components: {
        IconamoonArrowDown2Fill, Fast, Slow, Medium
    },
    props: {
      modelValue: Number
    },
    ...
    created(){
        ...
        this.dialogId = ['modal_select_gas', Date.now(), randomInt(1, 1000)].join('_')
