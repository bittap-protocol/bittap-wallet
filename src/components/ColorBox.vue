          ...
          <div class="detail">
            ...
            <div class="arrow">
                <div v-if="info.asset_id !== 'Base'" class="dropdown dropdown-end">
                    <div tabindex="0" role="button" class="btn px-2 py-0 m-0 min-h-fit h-auto border-none border-0">
                    ...
                    <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-[130px] px-0 py-4 shadow font-normal text-sm">
                        <li><a :href="'https://explorer.royllo.org/search?query='+info.asset_id" target="_blank">Token Detail</a></li>
                        <li v-if="info.asset_type === 0"><a @click="hideToken">Hide {{ info.asset_name }}</a></li>
            ...
            this.$root._main_confirm(
                ...
                { name: 'Cancel' },
                {
                    ...
                    handle: (action, closeModal) => {
                        closeModal()
                         // @ts-ignore
                        this.store.removeToken(this.asset_id)
                        this.$router.push('/')
                    },
                },
                ],
