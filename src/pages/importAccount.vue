    ...
    methods: {
        handlePaste(event: ClipboardEvent) {
            ...
            if (pastedData.split(/[\s\n]+/).length === 12) {
                ...
                const currentIndex = Array.from(
                    ...
                    targetInput.parentElement?.parentElement as HTMLElement
                )
                const words = pastedData.split(/[\s\n]+/)
                ...
                if (words.length === 1) {
                    // @ts-ignore
                    this.wordsForm[currentIndex] = words[0]
                    event.preventDefault()
                } else {
                    // 如果粘贴的是多个词，从当前输入框开始填充
                    for (
                        let i = 0;
                        i < words.length && currentIndex + i < 12;
                        i++
                    ) {
                        // @ts-ignore
                        this.wordsForm[currentIndex + i] = words[i]
                    }
                    event.preventDefault()
                }
            }
        ...
        async importAccountFromWords() {
            try {
                ...
                await this.store
                    .createNewUser(this.wordsForm.join(' '))
                    .finally(() => {
                        // @ts-ignore
